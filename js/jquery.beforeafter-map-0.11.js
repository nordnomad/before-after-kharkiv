...
(function($){
	$.fn.extend({ 
		...
		var randID =  Math.round(Math.random()*100000000);
			...
			$(obj).prepend('<div id="dragwrapper'+randID+'"><div id="drag'+randID+'"><img width="1" height="56" alt="handle" src="'+o.imagePath+'handle.gif" id="handle'+randID+'" /></div></div>'); // Create drag handle
			...
			$('#drag'+randID).width(2).height(mapHeight).css({'background':'#c00','position':'absolute','left':'0px'});	// Set drag handle CSS properties
			...
			{	
				...
				$('#showleft'+randID).css({'position':'absolute','left':'0px'}).click(function(){
					$('#before', obj).animate({width:mapWidth},o.linkDisplaySpeed);
				...
				$('#showright'+randID).css({'position':'absolute','right':'0px'}).click(function(){
					$('#before', obj).animate({width:0},o.linkDisplaySpeed);
			...
			function drag()
			{
				...
				$('#before').width( parseInt( $(this).css('left') ) + 4 );
				/*if (o.permArrows){
